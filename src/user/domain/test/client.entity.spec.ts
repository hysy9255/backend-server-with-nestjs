import { ClientEntity } from '../client.entity'; // adjust path if needed
...
describe('ClientEntity', () => {
  ...
  describe('createNew', () => {
    it('should create a new customer with generated id', () => {
      const customer = ClientEntity.createNew(userId, deliveryAddress);
  ...
  describe('fromPersistance', () => {
    it('should recreate customer with given id, userId, and address', () => {
      const id = uuidv4();
      const customer = ClientEntity.fromPersistance(
        id,
        userId,
        deliveryAddress,
      );
